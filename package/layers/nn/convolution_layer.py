...
import _mapped_convolution_ext._convolution as conv
...
class ConvolutionFunction(torch.autograd.Function):
    ...
    def forward(self, input, weight, bias, kernel_size, stride, padding,
        ...
        return conv.conv_forward(input, weight, bias, kernel_size[0],
                                 kernel_size[1], stride[0], stride[1],
                                 padding[0], padding[1], dilation[0],
                                 dilation[1])

    @staticmethod
    def backward(self, grad_output):
        input, weight, bias, kernel_size, stride, padding, dilation = self.saved_tensors
        grad_input, grad_weight, grad_bias = conv.conv_backward(
...
class Convolution(nn.Module):
    ...
    def forward(self, x):
        return ConvolutionFunction.apply(x, self.weight, self.bias,
